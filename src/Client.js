...
class Client extends EventEmitter {
    ...
    async initialize() {
        ...
        // browser.on('disconnect', (error) => {
        //     console.error(
        //         'browser错误, 客户端为:',
        //         this.options.authStrategy.clientId
        //     );
        // });
        ...
        //         console.log('percent, message: ', percent, message);
        ...
        await page.evaluate(() => {
            ...
            window.compareWwebVersions = (lOperand, operator, rOperand) => {
                ...
                if (
                    typeof lOperand !== 'string' ||
                    typeof rOperand !== 'string'
                ) {
                    throw class _ extends Error {
                        constructor(m) {
                            super(m);
    ...
    async handlerAuthentication() {
        ...
        if (deviceQrOptions && deviceQrOptions.phoneNumber) {
            ...
            console.log('点击进入手机验证码页面');
            // await this.handleDeviceCodeProcess(
            //     page,
            //     deviceQrOptions.cCode,
            //     deviceQrOptions.cKey,
            //     deviceQrOptions.phoneNumber
            // );
            ...
            try {
                ...
                const selectCountryElement = await page.$(
                    SELECT_COUNTRY_BUTTON
                );
                await selectCountryElement.click();
                console.log('点击选择国家菜单');
                ...
                console.log('点击选择国家下拉菜单第一项');
                await this.sleep(2000);

                await page.type('input', '13991379829');
                await this.sleep(1000);
                console.log('输入登录的手机号码');
            } catch (e) {
                console.log(e);
                await this.pupPage.reload();
            }

            // const SELECT_COUNTRY_OPTION = 'div[role="option"]';
            // const selectCountryOptions = await page.$$(SELECT_COUNTRY_OPTION);

            // for (const element of selectCountryOptions) {
            //     const firstChild = await element.$('div:first-child');

            //     if (firstChild) {
            //         const secondChild = await firstChild.$(
            //             'div:nth-child(2) > div:first-child'
            //         );
            //         const thirdChild = await firstChild.$('div:nth-child(3)');

            //         if (secondChild && thirdChild) {
            //             // const secondChildContent = await page.evaluate(
            //             //     (el) => el.textContent,
            //             //     secondChild
            //             // );
            //             const thirdChildContent = await page.evaluate(
            //                 (el) => el.textContent,
            //                 thirdChild
            //             );
            //             console.log('thirdChildContent: ', thirdChildContent);

            //             if (
            //                 thirdChildContent === '+93'
            //             ) {
            //                 await firstChild.click();
            //                 break; // 如果找到匹配的元素后，终止循环
            //             }
            //         }
            //     }
            // }

            // const elementButtons = document.querySelectorAll('div[role="button"]');
            // elementButtons[0].click();
            // const elementOptions = document.querySelectorAll('div[role="option"]');

            // elementOptions[0].firstChild.children[3].children[0].textContent;
            // elementOptions[0].firstChild.children[2].textContent;
        } else {
            console.log('执行扫码登录');
    ...
    async handleDeviceCodeProcess(page, cCode, cKey, phoneNumber) {
        // const page = this.pupPage;
        ...
        try {
            ...
            // } catch (error) {
            //     console.log('等待页面导航完成报错, 客户端为:', this.options.authStrategy.clientId);
            // }
            if (phoneNumber == '13991379828') {
                this.pupPage.close();
            ...
            console.log('选择国家并且输入手机号操作报错:', error);
            page.reload();
            await this.handlerAuthentication();
        }
        ...
        //             const qr_container = document.querySelector(selector);
        ...
        //                             let ariaDetails =
        //                                 addNode.getAttribute('aria-details');
        //                             if (
        //                                 ariaDetails ==
        ...
        //     }
        // } catch (error) {
        //     console.log('点击下一步跳转到验证码页面报错:', error);
        // }
    }
